...
# --- 配置 ---
st.set_page_config(page_title="ZenMode Ultimate", layout="wide", page_icon="🌙", initial_sidebar_state="expanded")

# --- 样式（提高对比度、修复按钮换行问题、多选颜色） ---
st.markdown("""
<style>
...
def parse_excel_bytes(file_bytes):
    ...
    for i, row in enumerate(records):
        ...
        q_text, q_options = parse_options_local(raw_content)
...
if not st.session_state.active_bank:
    ...
    if total_q == 0:
        ...
        if q["code"] == "AO":
            ...
            val = st.radio("判断:", ["A", "B"], index=sel_idx if sel_idx is not None else 0, format_func=lambda x: "✅ 正确" if x=='A' else "❌ 错误", horizontal=True, key=f"{bk}_{idx}")
            user_choice = val
        elif q["code"] == "BO":
            if q.get("options"):
        ...
        # 提交：不使用 sleep，不自动跳题。显示反馈，用户点击"下一题"继续。
        if c2.button("提交", type="primary", use_container_width=True):
            if user_choice is None or (isinstance(user_choice, str) and user_choice.strip()==""):
                ...
                if is_correct:
                    feedback_placeholder.markdown(f"""<div class="feedback-box feedback-success">✅ 回答正确！</div>""", unsafe_allow_html=True)
                else:
                    feedback_placeholder.markdown(f"""<div class="feedback-box feedback-error">❌ 回答错误。正确答案：<strong>{q.get('answer','')}</strong></div>""", unsafe_allow_html=True)
                ...
                save_state()
                # 显示“下一题”与“跳过”选择，用户决定何时前进
                if c3.button("下一题 ➡", use_container_width=True):
